...
async function addUser(req, res) {
  try {
    ...
    const db_res = await pool.query(
      ...
      [req.body.email_id, req.body.name, req.body.admin]
    );
    ...
    if (req.body.admin == 2) {
      ...
      str_query = str_query.concat(
        ' (project_id text, project_title text, professor_list text, project_grant integer, comment_time timestamp with time zone, pi text, co_pi text, dept text, fund_agency text, sanc_order_no text, sanctioned_date text, duration text, dos text, doc text, start_year text)'
      );
