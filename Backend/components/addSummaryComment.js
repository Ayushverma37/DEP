...
async function addSummaryComment(req, res) {
  try {
    ...
    query = query.concat(
      "_summary_comment_table VALUES ($1,$2,$3,current_timestamp,'NO')"
    );
    ...
    if (req.body.is_admin == 2) {
      query = 'UPDATE ';
      query = query.concat(req.body.project_id);
      query = query.concat('_summary_table set comm_flag = 1 where sr = ');
    ...
    for (var i in prof_emails) {
      ...
      query = query.concat(
        '_proj_list SET comment_time = current_timestamp where project_id = $1'
      );
...
module.exports = { addSummaryComment: addSummaryComment };
